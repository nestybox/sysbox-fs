...
// nestybox: shiftfs for debian stretch (kernel 4.9.0-7)
//
// note: debian stretch requires kernel patch to net/unix/af_unix.c
// (d_real_inode -> d_backing_inode) in order to support shiftfs.
// (kernel git commit beef5121f3a4d1566c8ab8cd99b4e001862048cf by
// Miklos Szeredi on 12/16/16: revert "af_unix: fix hard linked
// sockets on overlay")

// must not collide with anything in include/uapi/linux/magic.h
                                     ...
                                     unsigned int open_flags)
{
	...
	if (unlikely(real->d_flags & DCACHE_OP_REAL))
      return real->d_op->d_real(real, real->d_inode, open_flags);
	return real;
}
...
static int shiftfs_make_object(struct inode *dir, struct dentry *dentry,
                               ...
                               struct dentry *hardlink, bool excl, dev_t dev)
{
	...
	if (hardlink) {
      ...
      case S_IFCHR:
      case S_IFIFO:
         err = vfs_mknod(reali, new, mode, dev);
      }
	}
...
{
	mode |= S_IFREG;
	return shiftfs_make_object(dir, dentry, mode, NULL, NULL, excl, 0);
}
...
{
	mode |= S_IFDIR;
	return shiftfs_make_object(dir, dentry, mode, NULL, NULL, false, 0);
}

static int shiftfs_mknod(struct inode *dir, struct dentry *dentry,
                         umode_t mode, dev_t dev)
{
	return shiftfs_make_object(dir, dentry, mode, NULL, NULL, false, dev);
}

static int shiftfs_link(struct dentry *hardlink, struct inode *dir,
                        struct dentry *dentry)
{
	return shiftfs_make_object(dir, dentry, 0, NULL, hardlink, false, 0);
}

static int shiftfs_symlink(struct inode *dir, struct dentry *dentry,
                           const char *symlink)
{
	return shiftfs_make_object(dir, dentry, S_IFLNK, symlink, NULL, false, 0);
}
...
static int shiftfs_rm(struct inode *dir, struct dentry *dentry, bool rmdir)
{
   ...
   // nestybox: the ref count of the real dentry was implicitly set
   // to 1 in shiftfs_lookup(). That ref count remains unchanged during
   // shiftfs lookups (only shiftfs dentry ref counts are updated).
   // If we simply call vfs_unlink() below, the vfs will remove the
   // inode reference from the real dentry (which causes problems on
   // sequences such as open->unlink->write). To avoid this, we get
   // a reference to the real dentry.
...
static int shiftfs_permission(struct inode *inode, int mask)
{
   ...
   // XXX: nestybox: we are only checking the underlying inode
   // permissions with the shifted credentials. Should we also check
   // check the shiftfs inode permissions with the (non-shifted) task
   // credentials?
                           ...
                           struct kstat *stat)
{
	...
	struct dentry *real = inode->i_private;
	struct inode *reali = real->d_inode;
	struct shiftfs_super_info *ssi = (struct shiftfs_super_info *)dentry->d_sb->s_fs_info;
	const struct inode_operations *iop = reali->i_op;
	int err = 0;

	if (iop->getattr)
		err = iop->getattr(ssi->mnt, real, stat);
	else
...
static const struct inode_operations shiftfs_inode_ops = {
	...
	.mknod		= shiftfs_mknod,   // nestybox: add mknod support in shiftfs
