...
// nestybox: shiftfs for ubuntu 18.10 (kernel 4.18.0-17-generic)
                           ...
                           u32 request_mask, unsigned int query_flags)
{
	...
	struct inode *reali = real->d_inode;
	const struct inode_operations *iop = reali->i_op;
   struct shiftfs_super_info *ssi = (struct shiftfs_super_info *)path->dentry->d_sb->s_fs_info;
	struct path newpath = { .mnt = ssi->mnt, .dentry = real };
	int err = 0;
