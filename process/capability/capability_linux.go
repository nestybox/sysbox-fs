...
var (
	...
	capLastCap domain.Cap

	// Highest valid capability of the running kernel.
	CAP_LAST_CAP = domain.Cap(63)
...
func mkStringCap(c Capabilities, which domain.CapType) (ret string) {
	for i, first := domain.Cap(0), true; i <= CAP_LAST_CAP; i++ {
		if !c.Get(which, i) {
			continue
		}
		if first {
...
func mkString(c Capabilities, max domain.CapType) (ret string) {
	ret = "{"
	for i := domain.CapType(1); i <= max; i <<= 1 {
		ret += " " + i.String() + "=\""
		if c.Empty(i) {
			ret += "empty"
		} else if c.Full(i) {
...
func (c *capsV1) Get(which domain.CapType, what domain.Cap) bool {
	if what > 32 {
		return false
	}

	switch which {
	case domain.EFFECTIVE:
		return (1<<uint(what))&c.data.effective != 0
	case domain.PERMITTED:
		return (1<<uint(what))&c.data.permitted != 0
	case domain.INHERITABLE:
...
func (c *capsV1) getData(which domain.CapType) (ret uint32) {
	switch which {
	case domain.EFFECTIVE:
...
func (c *capsV1) Empty(which domain.CapType) bool {
	return c.getData(which) == 0
}

func (c *capsV1) Full(which domain.CapType) bool {
	return (c.getData(which) & 0x7fffffff) == 0x7fffffff
}

func (c *capsV1) Set(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		if what > 32 {
			continue
		}

		if which&domain.EFFECTIVE != 0 {
			c.data.effective |= 1 << uint(what)
		}
		if which&domain.PERMITTED != 0 {
			c.data.permitted |= 1 << uint(what)
		}
		if which&domain.INHERITABLE != 0 {
			c.data.inheritable |= 1 << uint(what)
		}
	}
}

func (c *capsV1) Unset(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		if what > 32 {
			continue
		}
...
func (c *capsV1) Fill(kind domain.CapType) {
...
func (c *capsV1) Clear(kind domain.CapType) {
	if kind&domain.CAPS == domain.CAPS {
		c.data.effective = 0
		c.data.permitted = 0
		c.data.inheritable = 0
...
func (c *capsV1) StringCap(which domain.CapType) (ret string) {
...
func (c *capsV1) Apply(kind domain.CapType) error {
	if kind&domain.CAPS == domain.CAPS {
...
func (c *capsV3) Get(which domain.CapType, what domain.Cap) bool {
	var i uint
	if what > 31 {
		i = uint(what) >> 5
		what %= 32
...
func (c *capsV3) getData(which domain.CapType, dest []uint32) {
	switch which {
	case domain.EFFECTIVE:
...
func (c *capsV3) Empty(which domain.CapType) bool {
	var data [2]uint32
	c.getData(which, data[:])
	return data[0] == 0 && data[1] == 0
}

func (c *capsV3) Full(which domain.CapType) bool {
	var data [2]uint32
	c.getData(which, data[:])
...
func (c *capsV3) Set(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		var i uint
		if what > 31 {
			i = uint(what) >> 5
		...
		if which&domain.BOUNDING != 0 {
			c.bounds[i] |= 1 << uint(what)
		}
		if which&domain.AMBIENT != 0 {
			c.ambient[i] |= 1 << uint(what)
		}
	}
}

func (c *capsV3) Unset(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		var i uint
		if what > 31 {
			i = uint(what) >> 5
...
func (c *capsV3) Fill(kind domain.CapType) {
	if kind&domain.CAPS == domain.CAPS {
...
func (c *capsV3) Clear(kind domain.CapType) {
	...
	if kind&domain.PERMITTED == domain.PERMITTED {
		c.data[0].permitted = 0
		c.data[1].permitted = 0
	}
	if kind&domain.INHERITABLE == domain.INHERITABLE {
		c.data[0].inheritable = 0
		c.data[1].inheritable = 0
	}
...
func (c *capsV3) StringCap(which domain.CapType) (ret string) {
	return mkStringCap(c, which)
}
...
func (c *capsV3) Apply(kind domain.CapType) (err error) {
	if kind&domain.BOUNDS == domain.BOUNDS {
		var data [2]capData
		...
		if (1<<uint(domain.CAP_SETPCAP))&data[0].effective != 0 {
			for i := domain.Cap(0); i <= CAP_LAST_CAP; i++ {
				if c.Get(domain.BOUNDING, i) {
					continue
				}
...
func (c *capsFile) Get(which domain.CapType, what domain.Cap) bool {
	var i uint
	if what > 31 {
		if c.data.version == 1 {
			return false
...
func (c *capsFile) getData(which domain.CapType, dest []uint32) {
	switch which {
	case domain.EFFECTIVE:
...
func (c *capsFile) Empty(which domain.CapType) bool {
	var data [2]uint32
	c.getData(which, data[:])
	return data[0] == 0 && data[1] == 0
}

func (c *capsFile) Full(which domain.CapType) bool {
	var data [2]uint32
	c.getData(which, data[:])
	if c.data.version == 0 {
...
func (c *capsFile) Set(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		var i uint
		if what > 31 {
			if c.data.version == 1 {
...
func (c *capsFile) Unset(which domain.CapType, caps ...domain.Cap) {
	for _, what := range caps {
		var i uint
		if what > 31 {
			if c.data.version == 1 {
...
func (c *capsFile) Fill(kind domain.CapType) {
	if kind&domain.CAPS == domain.CAPS {
...
func (c *capsFile) Clear(kind domain.CapType) {
	if kind&domain.CAPS == domain.CAPS {
...
func (c *capsFile) StringCap(which domain.CapType) (ret string) {
	return mkStringCap(c, which)
}

func (c *capsFile) String() (ret string) {
	return mkString(c, domain.INHERITABLE)
}
...
func (c *capsFile) Apply(kind domain.CapType) (err error) {
	if kind&domain.CAPS == domain.CAPS {
